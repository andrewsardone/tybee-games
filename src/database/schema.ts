...
export const games = sqliteTable('games', {
  ...
  playersIdx: index('idx_games_players').on(table.minPlayers, table.maxPlayers),
  durationIdx: index('idx_games_duration').on(table.minDuration, table.maxDuration),
  complexityIdx: index('idx_games_complexity').on(table.complexityLevel),
  strategyLuckIdx: index('idx_games_strategy_luck').on(table.strategyLuckRating),
}))

// Physical game copies (multiple copies per game)
export const gameCopies = sqliteTable('game_copies', {
  id: text('id').primaryKey(),
  gameId: text('game_id').notNull().references(() => games.id, { onDelete: 'cascade' }),
  copyNumber: integer('copy_number').notNull(),
  status: text('status', { enum: ['available', 'checked_out', 'maintenance', 'missing'] }).notNull().default('available'),
  condition: text('condition', { enum: ['excellent', 'good', 'fair', 'poor'] }).notNull().default('excellent'),
  ...
  availableIdx: index('idx_game_copies_available').on(table.gameId, table.status),
  uniqueGameCopy: index('idx_game_copies_unique').on(table.gameId, table.copyNumber),
}))

// Checkout tracking
export const checkouts = sqliteTable('checkouts', {
  id: text('id').primaryKey(),
  gameCopyId: text('game_copy_id').notNull().references(() => gameCopies.id, { onDelete: 'cascade' }),
...
export const staffUsers = sqliteTable('staff_users', {
  ...
  name: text('name').notNull(),
  role: text('role', { enum: ['staff', 'admin', 'manager'] }).default('staff'),
  isActive: integer('is_active', { mode: 'boolean' }).default(true),
