...
async function main() {
  ...
  if (!localDbPath) {
    ...
    console.error('This should be automatically set by the package.json script')
    process.exit(1)
  }
  ...
  try {
    ...
    console.error('2. Wrangler has created the local database (try running `npm run dev` first)')
    process.exit(1)
  }
}
